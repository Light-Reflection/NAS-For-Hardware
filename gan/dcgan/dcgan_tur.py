...
dataroot = '../data/your-dataset/aligned'
...
for epoch in range(num_epochs):
    ...
    # if (epoch+1)% 5 == 0:
    torch.save({'generator': netG.state_dict(),
            'discriminator': netD.state_dict()
            }, os.path.join('tur', 'model_{}.pt'.format(epoch)))
...
ims = [[plt.imsave('list_{}.pdf'.format(str(i)), np.transpose(i,(1,2,0)))] for i in img_list]
...
plt.imsave('./make_grid.pdf', np.transpose(vutils.make_grid(real_batch[0].to(device)[:64], padding=5, normalize=True).cpu(),(1,2,0)))
...
plt.imsave('./fakeimg.pdf',np.transpose(img_list[-1],(1,2,0)))
#plt.show()
