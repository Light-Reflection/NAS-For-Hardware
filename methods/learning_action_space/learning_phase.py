...
def train_classifier(X, y):
    ...
    print("valid_0's binary_error: ", model.best_score["valid_0"][params["metric"]])
    return model
...
class LearningPhase:
    def __init__(self, X, y, height, h):
        ...
        if self.height > 0:
            ...
            build_trial = 0
            while self.model.best_score["valid_0"]["binary_error"] >= 0.5 + max(0, (build_trial-200.0)/1000*0.3): 
                 build_trial += 1
                 print('----------------------rebuild'+ str(build_trial)+'-----------------------------')
                 self.model =  train_classifier(X, y >= self.m)
